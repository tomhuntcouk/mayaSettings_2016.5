...
def CharSetup_FKCTRL_Type_Values():
    But0   = ['Curve Shape',"Does not create an FK controller, only creates a curve shape"]
    But1   = ['Standard FK',"Standard FK Controller"]
    But2   = ['Node Swap',"Create an FK controller that drives other Joints, 1st selected object acts as parent of other selected joints/nodes - (child nodes will not have a control curve)- . also has a value to show all child nodes or to hide/scale to 0. this is good for swapping of body parts, meshes of a rig (like different shape eyes.weapons.. ect) each body part should be driven/skinned 100 percent by one of the child joints"]
    But3   = ['Squash Strech',"Squash & Stretch. for a basic squash and strech for - fk control. by adjusting the scale value of CTRL that effects placement of other controls" ]
    But4   = ['Shadow',"Create an FK controller that will be driven to some extent by the hips/Cog of the character, but not effected on the Translate Y" ]
    ButtonList = [But0,But1,But2,But3,But4]
    RCollection = "RadioCollection_FKCTRL_Types"    

    return [RCollection,ButtonList]
    
def CharSetUp_GetFKCTRL():
    CTRLType = cmds.checkBox("FKCurve_FKCTRL_SELECT",q=True,v=True)
    RCollection =  CharSetup_FKCTRL_Type_Values()[0]
    CTRLType = cmds.radioCollection(RCollection,q=True,sl=True)
    print CTRLType
    cmds.select("kdkdkd")
    return CTRLType
...
def CharSetUp_CTRLMultiNode_Create():
    CTRLType            = CharSetUp_GetFKCTRL()
    Rigging.MultiJointCTRL()
...
def CharSetUp_LabelJoint_Values():
    ...
    Left        = [LabelJoints + "Left","Left"]
    Right       = [LabelJoints + "Right","Right"]
    Prefix      = [LabelJoints + "Prefix","Prefix"]
    Suffix      = [LabelJoints + "Suffix","Suffix"]
    return [LabelJoints,CheckBox,Left,Right,Prefix,Suffix]

def CharSetUp_LabelJoint_OnOff():
    CheckBox    = CharSetUp_LabelJoint_Values()[1]
    Left        = CharSetUp_LabelJoint_Values()[2]
    Right       = CharSetUp_LabelJoint_Values()[3]
    Prefix      = CharSetUp_LabelJoint_Values()[4]
    Suffix      = CharSetUp_LabelJoint_Values()[5]
    List        = CharSetUp_LabelJoint_Values()
    ONOFF = cmds.checkBox(List[1],q=True,v=True)
...
def CharSetUp_LabelJoints_Create(ButtonColor):
    ...
    CharSetup_LabelTextField(Left[0],Left[1],"L","Left Naming Convention - to determine side for labeling joints - example (L_Leg_JNT) - type in 'L_' to determine side for labeling",False)
    CharSetup_LabelTextField(Right[0],Right[1],"R","Right Naming Convention - to determine side for labeling joints - example (R_Leg_JNT) - type in 'R_' to determine side for labeling",False)
    cmds.setParent( '..' )
...
def CharSetup_JointLabel():
    ...
    CheckBox    = CharSetUp_LabelJoint_Values()[1]
    Left        = [Locations[RadioSelect-1],2,CharSetUp_LabelJoint_Values()[2][0]]
    Right       = [Locations[RadioSelect-1],1,CharSetUp_LabelJoint_Values()[3][0]]
    PrefixText  = [Locations[0],CharSetUp_LabelJoint_Values()[4][0]]
    SuffixText  = [Locations[2],CharSetUp_LabelJoint_Values()[5][0]]
    List        = [Left,Right,PrefixText,SuffixText]

    Skeleton.UpdateJointLabel(List)
...
def GetLimbs_Selected():
    import string
    Status     = cmds.checkBoxGrp("checkBox_IKLimbs",q=True,va2=True)
    Names      = cmds.checkBoxGrp("checkBox_IKLimbs",q=True,la2=True)
    LIMBS = []
    for S in range(len(Status)):
    	if Status[S]:
            LIMBS.append(Names[S].rstrip("s"))
    return LIMBS
...
def RiggingIK_Create():
    ...
    print "LIMBS : ", LIMBS
    for Limb in LIMBS:
        #may need to update this.. but if it's the arm not proxy Jnts for now.. 
        if Limb == "Arm":
            Rigging_IK.IK_Setup(Limb,CharType,IncludeFK,False,ExtraParentSwitchLimbs,ZeroJoints)
        else:
            Rigging_IK.IK_Setup(Limb,CharType,IncludeFK,RollFootFix,ExtraParentSwitchLimbs,ZeroJoints)
...
def CharSetUp_Create_FKControl():
    CharSetUp_SceneUnits_Set()
    CTRLType            = CharSetUp_GetFKCTRL()
    if (CTRLType):
...
def CharSetup_UI_RadioCollection(RCollection,ButtonList,Select=0): 
    cmds.radioCollection(RCollection)
    for RButton in ButtonList:
        Name = RButton[0]
        Note = RButton[1]        
        temp = cmds.radioButton(RCollection + "_" + Name.replace(" ",""),label=Name,cl=RCollection,ann=Note)
    cmds.radioCollection( RCollection, edit=True, select=RCollection + "_" +  ButtonList[Select][0].replace(" ","") )    
     
def CharSetup_Rigging_UI_Create_FKCTRLS_Types():
    #cmds.button(l='Create CTRL MultiNode', c='UI.CharSetUp_CTRLMultiNode_Create()',bgc=(0.556,0.804,0.611))
    Name = "CTRL Shape"
    List = Curves.Type()
    CharSetUp_DropDownMenu(Name,List,"Select curve shape type for creating control curve")
    #cmds.rowColumnLayout(numberOfRows=4)#,cal=[(1,"center"),(2,"center"),(2,"center"),(2,"center")],cs=[(1,2),(2,2),(3,2)])
    
    cmds.text("TextField_FKCTRL_Type",label="Select FK Control Type",align='center',ann="Select the type of FK Control to create")
    RCollection = CharSetup_FKCTRL_Type_Values()[0]
    ButtonList  = CharSetup_FKCTRL_Type_Values()[1]
    CharSetup_UI_RadioCollection(RCollection,ButtonList,1)
    
    #cmds.setParent( '..' )
    

    Title = "Standard FK CTRL"
...
def CharSetUp_Rigging_UI_Create_FKCTRLS(ButtonColor):
    
    #create CTRL Curves.. 
    FrameLayout = cmds.frameLayout(label='Create FK CTRLS',cl = True, collapsable = True,bv=True)
    CharSetup_Rigging_UI_Create_FKCTRLS_Types()    
    cmds.separator(h=1)
    ...
    cmds.separator(h=1)
    cmds.button(l='Create FK CTRL', c='UI.CharSetUp_Create_FKControl()',bgc =ButtonColor,ann="run to creat Control curve for rigging nodes.. selected Joints or already selected rigged Control curves: look at the available options for rigging.")
    cmds.separator(h=1)
...
def CharSetUp_Rigging_UI_RigBase(ButtonColor):
    ...
    cmds.button(l='Rig Base - VisSwitch', c='Rigging.BASECTRL_VisibilitySwitch("CHAR_CTRL","CTRLS_GRP")',bgc =ButtonColor,ann="Adds, visibilit switch for controls, if base control does not have visibility switch setup")
    cmds.button(l='VisSwitch CTRL & Nodes/Attributes', c='Rigging.BASECTRL_VisibilitySwitch()',bgc =ButtonColor,ann="May need to add better description, Adds visibility switch to 1st selected node & controls all other nodes selected visibility")
...
def CharSetUp_Rigging_UI(UIName):
    ...
    CharSetUp_Rigging_UI_Create_FKCTRLS(ButtonColor)
...
def CharSetUp_Settings_UI(UIName):
    ...
    cmds.button(l='Skinned Verts over Influence Limit', c='Skinning.CheckMaxInfluence()',ann="temp")
