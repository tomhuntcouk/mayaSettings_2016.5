...
class IdToNameEntry(object):
    ...
    def __repr__(self):
        return "IdToNameEntry({0},{1})".format(self.id, self.name)
...
class TreeViewIDList(object):
    ...
    def setItems(self, newItems, selectedID):
        ...
        collapsedItems = set([item.id for item in self.items if self.isCollapsed(item.id)])
        ...
        if self.items != newItems:
            ...
            for i in newItems:
                cmds.treeView(self.control, e=True, addItem=(i.id, "" if i.parent is None else str(i.parent)))
                cmds.treeView(self.control, e=True, displayLabel=(i.id, i.displayName), displayLabelSuffix=(i.id, i.suffix), expandItem=(i.id, i.id not in collapsedItems))
    ...
    def setCollapsed(self, itemId, isCollapsed):
        cmds.treeView(self.control, e=True, expandItem=(itemId, not isCollapsed))
...
class LayersTreeView(TreeViewIDList):
    ...
    def handleDragDropCommand(self, itemList, prevParents, prevIndexes, newParent, newIndexes, itemBefore, itemAfter):
        self.itemDropped(itemList, newParent, itemBefore, itemAfter)
...
class LayerListsUI:
    ...
    def updateLayerList(self):
        ...
        layers = list(self.data.mll.listLayers())

        currLayer = self.data.getCurrentLayer()
            
        newItems = []
        for layerId, layerName, layerParent in layers:
            entry = IdToNameEntry(itemId=str(layerId))
            entry.name = layerName
            entry.displayName = layerName
            entry.suffix = "" if self.data.getLayerEnabled(layerId) else " [OFF]"

            entry.parent = layerParent            
    ...
    def updateInfluenceList(self):
        ...
        if self.data.getCurrentLayer() > 0:
            newItems.append(IdToNameEntry(itemId=NamedPaintTarget.MASK, name="[Layer Mask]"))
            if self.data.mll.hasDqBlendTarget(self.data.mll.getCurrentLayer()):
                newItems.append(IdToNameEntry(itemId=NamedPaintTarget.DUAL_QUATERNION, name="[Dual Quaternion Weights]"))
        ...
        for name, displayName, influenceId in zip(names, displayNames, ids):
            if filter.isMatch(displayName):
                newItems.append(IdToNameEntry(itemId=str(influenceId), name=name, displayName=displayName))
    ...
    def layerDropped(self, layers, newParent, itemBefore, itemAfter):
        ...
        def convertToNone(item):
            return (None if item == '' else item)
        
        newParent = convertToNone(newParent)
        itemBefore = convertToNone(itemBefore)
        itemAfter = convertToNone(itemAfter)
        
        mll = LayerDataModel.getInstance().mll

        def itemTitle(itemId):
            if itemId is None or itemId == '':
                return "<none>"
            return mll.getLayerName(itemId)
        
        with mll.batchUpdateContext():
            # first, order layers by index, and start the "drop" with lowest index
            layers = sorted(layers, key=mll.getLayerIndex, reverse=True)
...
#                itemBefore = newParent
            ...
            for layer in layers:
                mll.setLayerParent(layer, newParent)
    ...
    def createLayerListsUI(self, parent):
        ...
        inflDisplay = self.controls.influenceDisplay = InfluencesTreeView(allowMultiSelection=True)
        inflDisplay.onSelectionChanged.addHandler(self.execInfluenceSelected)
        ...
        form.attachForm(inflDisplay.control, None, Constants.MARGIN_SPACING_HORIZONTAL, 0, 0)
        form.attachControl(inflDisplay.control, label, 3, None, None, None)
    ...
    def getInfluencesList(self):
        return self.controls.influenceDisplay
