...
class BaseUIWrapper(object):
    ...
    def __init__(self,uiCommand,annotation=''):
        ...
        # track enabled/disabled in it's own context
        self.selfEnabled = True 
        pass
...
class DropDownField(ModelUIWrapper):
    ...
    def addOption(self,optionName):
        cmds.menuItem(parent=self.field,label=optionName)
    ...
    def clear(self):
        items = cmds.optionMenu(self.field, q=True, itemListLong=True)
        if items:
            cmds.deleteUI(items)
...
class FormLayout:
    ...
    def attachControl(self,control,targetControl,top=None,right=None,bottom=None,left=None):
        def a(value,name):
            cmds.formLayout(self.layout,e=True,ac=(control,name,value,targetControl))
        self.repeatTBLR(a, top, right, bottom, left)

    def attachControlOpposite(self,control,targetControl, top,right,bottom,left):
        def a(value,name):
            cmds.formLayout(self.layout,e=True,attachOppositeControl=(control,name,value,targetControl))
