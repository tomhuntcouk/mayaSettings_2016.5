...
def displayError(message):
    ...
    confirmDialog(title='NgSkinTools: Error', message=str(message), button=['Ok'], defaultButton='Ok')
...
class Utils:
    ...
    @staticmethod
    def createGlobalPythonProcedureInvocation(pythonMethod,args=()):
        '''
        as Autodesk does not support method pointers in some API methods (e.g. workspaceControl#uiScript in Maya2017),
        using this hack instead.
        '''
        procName = "ngSkinToolsProc%i%s" % (id(pythonMethod),pythonMethod.__name__)
        sys.modules['__main__'].__dict__[procName] = pythonMethod
        pythonArgs = ','.join(map(lambda a: '\'"+$%s+"\'' %a[1],args))
        return '{0}({1})'.format(procName,pythonArgs)
    ...
    def getMayaVersion():
        ...
        if Utils.CURRENT_MAYA_VERSION is None:
            ...
            # thanks to 2018 maya, format now changed from 201700 into 20180000
            # let's for now use only the old format
            if Utils.CURRENT_MAYA_VERSION > 1000000:
                Utils.CURRENT_MAYA_VERSION = Utils.CURRENT_MAYA_VERSION/100
    ...
    def loadPlugin():
        ...
        if not Utils.isPluginLoaded():
            displayError("Failed to load the plugin. This is often a case-by-case issue - contact support.")
            return
            
        if Utils.getPluginVersion()!=version.pluginVersion() and not Utils.DEBUG_MODE:
            displayError("Invalid plugin version detected: required '%s', but was '%s'. Clean reinstall recommended." % (version.pluginVersion(), Utils.getPluginVersion()))
...
import ngSkinTools
Utils.DEBUG_MODE = ngSkinTools.DEBUG_MODE