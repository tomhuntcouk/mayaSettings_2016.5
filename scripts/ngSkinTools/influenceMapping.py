...
class NameMatchingRule:
    ...
    def reversePath(self,path):
        ...
        self.reversePathCache[path] = result
        return result
    ...
    def testCandidate(self,destination):
        ...
        for source,dest,(scoreIndex,scoreValue) in izip(self.sourceNameParts,candidatePath,enumerate(self.bestMatchScore[:])):
            ...
            # if current candidate matched already,just continue filling in it's score
            if self.match == destination:
                self.bestMatchScore[scoreIndex] = currentScore
                continue    
            
            if currentScore==0:
                # current step not matched?
                return
...
class DistanceMatchRule:
    ...
    def vectorDistance(self,vFrom,vTo):
        return (vTo[0]-vFrom[0])**2+(vTo[1]-vFrom[1])**2+(vTo[2]-vFrom[2])**2

        # will be more readable than a long "if" statement

    def isBetterMatch(self, distanceSquare, destination):
        if distanceSquare > self.maxThreshold**2:
            return False

        if self.bestMatchDistance is None:
            return True

        distanceDiff = self.bestMatchDistance - distanceSquare
    ...
    def testCandidate(self,destination):
        ...
        distance = self.vectorDistance(self.matchPosition, destination.pivot)


        if self.isBetterMatch(distance,destination):
            self.match = destination
